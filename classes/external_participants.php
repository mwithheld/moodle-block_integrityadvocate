...
trait external_participants {

    /**
     * Get participant-level info (i.e. no session info) for the listed $userids.
     *
     ...
     */
    public static function get_participants(string $appid, int $courseid, int $moduleid, array $userids): array {
        global $USER;
        $fxn = __CLASS__ . '::' . __FUNCTION__;
        $debug = true || Logger::do_log_for_function($fxn);
        $debugvars = $fxn . "::Started with \$appid={$appid}; \$courseid={$courseid}; \$moduleid={$moduleid}; \$userids=" . ia_u::var_dump($userids);
...
//                break;
     ...
     */
    private static function get_participants_parameters(): \external_function_parameters {
        return new \external_function_parameters(
                [
            'appid' => new \external_value(PARAM_ALPHANUMEXT, 'appid'),
            'courseid' => new \external_value(PARAM_INT, 'courseid'),
            'moduleid' => new \external_value(PARAM_INT, 'moduleid'),
            // Usage: new \external_multiple_structure($content, $desc, $required, $default).
            'userids' => new \external_multiple_structure(new external_value(PARAM_INT, 'The request ID', VALUE_REQUIRED)),
                ]
        );
    }

    /**
     * Describes the get_participants return value.
     *
     * @return external_single_structure
     */
    private static function get_participants_returns(): \external_function_parameters {
