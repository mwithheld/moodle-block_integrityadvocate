...
class process_integrityadvocate extends \core\task\scheduled_task {
    ...
    public function execute() {
        ...
        if (ia_u::get_completion_setup_errors()) {
            $debug && ia_u::log(__FILE__ . '::' . __FUNCTION__ . '::Completion is not set up at the site level, so skip this task.');
            return true;
        }
        ...
        $debug && ia_u::log(__FILE__ . '::' . __FUNCTION__ . '::For $lastruntime=' . $lastruntime);
        ...
        $debug && ia_u::log(__FILE__ . '::' . __FUNCTION__ . '::Built params=' . print_r($params, true));
        ...
        if ($debug) {
            ...
            // Disabled on purpose: ia_u::log(__FILE__ . '::' . __FUNCTION__ . '::blockinstances=' . print_r($blockinstances, true));.
        }
        ...
        foreach ($blockinstances as $b) {
            ...
            $debug && ia_u::log(__FILE__ . '::' . __FUNCTION__ . "::{$debuglooplevel1}:Looking at {$debugblockidentifier}");

            $courselastmodified = ia_u::get_course_lastaccess($courseid);
            ...
            if (ia_u::get_completion_setup_errors($courseid)) {
                $debug && ia_u::log(__FILE__ . '::' . __FUNCTION__ .
            ...
            mtrace($msg);
            $debug && ia_u::log(__FILE__ . '::' . __FUNCTION__ . "::$msg");
            ...
            if (empty($participants)) {
                ...
                mtrace($msg);
                $debug && ia_u::log(__FILE__ . '::' . __FUNCTION__ . "::{$debuglooplevel1}:{$debugblockidentifier}: {$msg}, so skip it");
                continue;
            }
